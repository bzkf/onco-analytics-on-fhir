...
```sh
curl -L -O https://github.com/bzkf/diz-in-a-box/releases/download/v1.3.0/air-gapped-installer.tgz
```
...
```sh
helm upgrade --install --wait --timeout=10m --version=1.3.0 prerequisites oci://ghcr.io/bzkf/diz-in-a-box/charts/prerequisites
...
```sh
helm upgrade --install --wait --timeout=10m --version=1.3.0 diz-in-a-box oci://ghcr.io/bzkf/diz-in-a-box/charts/diz-in-a-box
