...
codes = ",".join(
    "http://fhir.de/CodeSystem/bfarm/icd-10-gm|" + c
    for c in condition_df["icd_10"]
)
...
condition_patient_df = search.steal_bundles_to_dataframe(
    ...
    fhir_paths=[
        ...
        ("patient_id", "Patient.identifier.where(type.coding.where(system='http://terminology.hl7.org/CodeSystem/v2-0203' and code='MR').exists()).value"),
...
condition_df["patient_id_from_ref"] = (
    condition_df["patient_ref"]
...
# merging deceased columns into one column 'deceased' (true if deceased_dateTime is set OR deceased_boolean is true)
...
merged_df["deceased"] = merged_df["deceased_boolean"] | merged_df["deceased_dateTime"].notna()
...
final_df = final_df[['patient_ref', 'patient_id','icd_code','diagnosis_recordedDate','loinc_code','loinc_display','lab_value','lab_unit','lab_dateTime','deceased_dateTime','deceased_boolean','deceased']]
